...
const AllCars = () => {
  ...
  const [priceRange, setPriceRange] = useState([1000, 10000000]); // Price range state
  ...
  const MIN_PRICE = 1000; // Minimum price
  ...
  useEffect(() => {
    const fetch = async () => {
      const res = await axios.get("https://highline-cars-backend.onrender.com/api/v1/car/get-all-cars");
